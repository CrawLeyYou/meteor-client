    ...
    private enum State {
        ...
        Restock {
            ...
            protected void tick(HighwayBuilder b) {
                ...
                if (b.restockTask.pickaxes) slotsPulled += countSlots(b, itemStack -> itemStack.is(ItemTags.PICKAXES)) - b.savePickaxes.get();
                if (b.restockTask.food) slotsPulled += countSlots(b, itemStack -> itemStack.has(DataComponents.FOOD) && !Modules.get().get(AutoEat.class).blacklist.get().contains(itemStack.getItem()));
            ...
            private void setShulkerPredicate(HighwayBuilder b) {
                shulkerPredicate = itemStack -> {
                    ...
                    for (ItemStack stack : ITEMS) {
                        ...
                        if (b.restockTask.food && stack.has(DataComponents.FOOD) && !Modules.get().get(AutoEat.class).blacklist.get().contains(stack.getItem())) return true;
                    }
        ...
        DefuseCrystalTraps {
             ...
             *
             * @see meteordevelopment.meteorclient.mixin.MinecraftClientMixin#wrapStopUsing(MultiPlayerGameMode, Player)
             */
            @Override
            protected void tick(HighwayBuilder b) {
                if (cooldown > 0) {
        ...
        protected void place(HighwayBuilder b, MBPIterator it, int slot, State nextState) {
            ...
            for (MBlockPos pos : it) {
                ...
                if (pos.getBlockPos().distToCenterSqr(b.mc.player.getEyePosition()) > b.placeRange.get() * b.placeRange.get()) continue;
